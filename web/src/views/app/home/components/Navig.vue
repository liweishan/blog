<template>
  <div class="navig-box">
    ...
    <div class="cover" v-show="showFlag"></div>
    <transition name="bounce" @after-enter="animationEnter" @after-leave="animationEnter">
      <div class="navig" v-show="showFlag"></div>
    </transition>
    <ul v-show="showFlag" class="menu">
      <li v-for="(v, i) of navList" :key="i" @click="handLink(v.path)">
        <a href="javascript:;">{{ v.meta.title }}</a>
      </li>
    </ul>
...
export default {
  ...
  computed: {
    ...
    navList() {
      const { navs } = this
      const list = navs.reduce((arr, v) => {
        if (v.meta && v.meta.isHome) {
          arr.push(v)
        }
        return arr
      }, [])
