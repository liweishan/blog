...
import { editArt } from '@/api/admin'
import { getCat, getArtDetails } from '@/api/blog'
  ...
  methods: {
    ...
    initForm(info) {
      ...
      for (const [k, v] of Object.entries(rules)) {
        ...
        this.$set(v[1], 'initialValue', info[k])
      }
      delete rules.file[1].initialValue
    },
    ...
    async requestArtDetails() {
      ...
      try {
        const res = await getArtDetails(query)
        const { info } = res.data
        this.initForm(info)
      } catch (err) {
        // console.log(err)
